...
namespace Teddy
{
	...
	void ScriptingEngine::InitMono()
	{
		...
		MonoClass* monoClass = mono_class_from_name(assemblyImage, "Teddy", "Main");
		if (!monoClass) {
			std::cerr << "Failed to retrieve MonoClass for 'Teddy.Main'.\n";
			return;
		}
		...
		MonoString* monoString = mono_string_new(s_Data->AppDomain, "Hello Baby");
		MonoMethod* printCustomMessageFunc = mono_class_get_method_from_name(monoClass, "PrintCustomMessage", 1);
		void* stringParam = monoString;
		mono_runtime_invoke(printCustomMessageFunc, instance, &stringParam, nullptr);


		MonoMethod* printSomeThingFunc = mono_class_get_method_from_name(monoClass, "printSomeThing", 1);
		mono_runtime_invoke(printSomeThingFunc, instance, &stringParam, nullptr);
