...
namespace Teddy
{
	...
	std::string OpenGLShader::ReadFile(const std::string& filePath)
	{
		...
		{
			TD_CORE_WARN("Could Not Open the Shader File, FilePath: {0}", filePath);
		}
	...
	void OpenGLShader::Compile(std::unordered_map<GLenum, std::string>& shaderSource)
	{
		...
		TD_CORE_ASSERT(shaderSource.size() <= 2, "We Only upport two ahader at thi point.");
		...
		// Attach our shaders to our program
			...
			glShaderIDs[glShaderIDInd++] = shader;
		}
